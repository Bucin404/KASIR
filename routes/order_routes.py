...
def generate_qr():
    ...
    try:
        ...
        current_app.logger.info(f"Generated QR for table {table_number}: {qr_url}")
        ...
        current_app.logger.error(f"Error generating QR page: {e}")
...
def generate_qr_api(table_number):
    ...
    try:
        ...
        return jsonify({
            'success': False,
            'message': f'Gagal generate QR: {str(e)}'
        }), 500
